...
class LocationService {
     ...
     */
    static details(locationId) {
        return api_base_1.default.request(`locations/${locationId}/`).then((details) => new Location_1.default(details));
    }
    /**
     ...
     */
    static getDesks(location) {
        return api_base_1.default.request(`locations/${location.id}/desks`).then((response) => {
            if (!response.desks) {
                throw new Error(`Desk list response was invalid - ${response}`);
            }
            return response.desks.map(each => new Desk_1.default(each));
        });
    }
    /**
     ...
     * wrong.
     */
    static getInputFields(location) {
        let locationId = null;
        // Get the location's ID
        if (location instanceof Location_1.default) {
        ...
        return api_base_1.default.request(`locations/${locationId}/input-fields`)
